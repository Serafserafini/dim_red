...
`dim_red/__init__.py` lazily imports `soap`, `fetch`, `analysis`, and `vae` via module-level `__getattr__` (PEP 562) so that importing the base package doesn't require optional heavy dependencies (dscribe, mp_api, jax, flax, etc.) unless those submodules are actually used. `vae/__init__.py` does the same lazy pattern internally for its own symbols. When adding new public symbols to either package, follow this lazy-import pattern rather than importing eagerly at module top.
