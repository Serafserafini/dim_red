...
def test_family_only_batch_has_expected_size_and_family_counts():
    family_ids = np.array([0] * 10 + [1] * 30 + [2] * 3)
    rng = np.random.default_rng(0)

    idx = balanced_batch_indices(family_ids, None, P=None, K=3, S=None, rng=rng)

    assert len(idx) == 3 * 3  # 3 families (all, P=None) x K=3
    counts = {f: int((family_ids[idx] == f).sum()) for f in (0, 1, 2)}
...
def test_p_larger_than_available_families_is_clamped_with_warning(caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="dim_red.pipeline"):
        idx = balanced_batch_indices(family_ids, None, P=5, K=2, S=None, rng=rng)
...
def test_sampling_without_replacement_when_pool_is_large_enough():
    ...
    idx = balanced_batch_indices(family_ids, None, P=1, K=50, S=None, rng=rng)
...
def test_sampling_with_replacement_when_pool_is_too_small():
    ...
    idx = balanced_batch_indices(family_ids, None, P=1, K=10, S=None, rng=rng)
...
def test_iter_balanced_batches_slices_arrays_consistently_with_chosen_indices():
    ...
    batches = iter_balanced_batches(
        (X, family_ids), family_ids, None, P=None, K=3, S=None, n_batches=1, rng=rng
    )
